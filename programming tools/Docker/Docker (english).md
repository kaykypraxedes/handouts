...
**Docker** is a platform for building, distributing, and running applications in isolated environments called **containers**. The application is packaged with its dependencies and basic configurations, reducing differences between development, testing, and runtime environments.
...
When the tag is omitted, Docker uses `latest`. This name is only a conventional, mutable tag: it does not guarantee that the image is the most recent version chronologically or that it will continue to point to the same content in the future. A *digest*, such as `image@sha256:{value}`, identifies specific content.

## Command structure:

The canonical CLI form generally follows the pattern `docker {object} {command} {options} {arguments}`. Some commands also have shorter historical aliases. For example, `docker image ls` can be written as `docker images`, `docker container ls` as `docker ps`, `docker image rm` as `docker rmi`, and `docker container rm` as `docker rm`.

Both forms are valid. In this summary, the object-oriented form is used as the main one; the aliases are grouped in this note to avoid duplicating each command.

## Search and information:

Because the CLI has many options, this summary presents only the most important ones. The built-in documentation and official references should be consulted for specific cases.
...
- `systemctl {action} docker.service` - Manages the Docker service. Important actions: `start`, `stop`, `restart`, `status`, `enable`, and `disable`.

- `journalctl {options} -u docker.service` - Queries the service logs. Important options: `-f` follows new messages, `-b` limits output to the current boot, and `--since` defines the period.
...
In the traditional (*rootful*) configuration, the daemon runs as `root` and creates a socket accessible by `root` and the `docker` group. The need for `sudo` comes from the permission on this socket, not simply from the fact that images and containers are normally stored in `/var/lib/docker`. The data directory can also be changed through the daemon configuration.

To allow the current user to access the daemon without `sudo`:
...
The new group membership applies only to new processes. The recommended procedure is to fully log out of the user session and log in again. As an immediate alternative, `newgrp docker` starts a new shell using `docker` as the primary group.

Access can be checked with:

```bash
$ docker container run --rm hello-world
```

If previous commands were run with `sudo`, the `~/.docker` directory may have become owned by `root`, causing errors when loading the client configuration. In this case, its ownership can be corrected with:

```bash
$ sudo chown -R "$USER":"$(id -gn)" "$HOME/.docker"
```

> The `docker` group grants privileges equivalent to those of `root`. A user who can control the daemon can mount *host* directories, start privileged containers, and modify the system. Therefore, only trusted users should belong to this group.
...
- `docker image ls {options} [{repository}[:{tag}]]` - Lists images stored locally. Important options: `-a` includes intermediate images, `-q` shows only identifiers, `--filter` filters, and `--format` customizes the output.

- `docker image inspect {options} {images}` - Shows detailed information in JSON, such as identifiers, tags, layers, architecture, and configuration. The `--format` option extracts specific fields.

- `docker image history {options} {image}` - Shows the layers and commands that formed an image. Important options: `--no-trunc`, `--human`, and `--format`.

## Transfer:

- `docker image pull {options} {name}[:{tag}|@{digest}]` - Downloads an image from a registry. Important options: `-a` downloads all tags, and `--platform` selects the platform.
...
- `docker image push {options} {name}[:{tag}]` - Pushes an image to the registry indicated by its name. Important options: `-a` pushes all tags, and `--platform` selects a specific platform.

- `docker login {options} [{registry}]` - Authenticates the client with a registry. Important options: `-u` specifies the user, and `--password-stdin` receives the credential through standard input without placing it directly in the shell history.
...
- `docker image save {options} {images}` - Saves one or more images, including layers and references, to a TAR file. The `-o {file}` option defines the output file.

- `docker image load {options}` - Loads images and tags from a TAR file created by `save`. The `-i {file}` option defines the input file; without it, the command reads standard input.
...
- `docker image rm {options} {images}` - Removes local image references. The `-f` option forces the operation, and `--no-prune` prevents the removal of untagged parent layers.
...
```bash
$ docker container run -d --name server \
    -p 127.0.0.1:8080:80 \
...
- `docker container ls {options}` - Lists containers. With no options, it shows only running containers; `-a` includes stopped containers, `-q` shows only identifiers, `--filter` filters, and `--format` customizes the output.

- `docker container inspect {options} {containers}` - Shows the configuration and state of one or more containers in JSON. The `--format` option selects fields.

- `docker container stats {options} [containers]` - Shows CPU, memory, network, and I/O usage. The `--no-stream` option produces only one reading.
...
- `docker container start {options} {containers}` - Starts stopped containers. Important options: `-a` attaches output, and `-i` attaches input.

- `docker container stop {options} {containers}` - Requests a graceful shutdown and forces termination after the timeout. Important options: `-s` selects the initial signal, and `-t` defines the waiting time.

- `docker container kill {options} {containers}` - Immediately sends a signal; the default is `SIGKILL`. The `-s` option selects another signal.

- `docker container restart {options} {containers}` - Stops and starts the containers again. Important options: `-s` and `-t` control the stop.

- `docker container rm {options} {containers}` - Removes containers. Important options: `-f` forces removal of a running container, and `-v` removes associated anonymous volumes.
...
- `docker container logs {options} {container}` - Shows the captured output of the container process according to the *logging driver*. Important options: `-f` follows new messages, `--tail` limits the last lines, `--since` and `--until` define the period, and `-t` shows timestamps.

- `docker container exec {options} {container} {command} {arguments}` - Starts a new process inside a running container. Important options: `-it` creates an interactive session, `-u` defines the user, `-e` adds variables, and `-w` defines the working directory.
...
A **Dockerfile** is a text file that reproducibly describes how to build an image. Its instructions are processed in order and generally produce reusable layers. When an instruction and its dependencies do not change, the builder can reuse the corresponding layer *cache*.
...
- `docker image build {options} {context}` - Builds an image from a Dockerfile. Important options: `-t {name}:{tag}` assigns a reference, `-f {file}` selects another Dockerfile, `--no-cache` ignores the *cache*, `--pull` attempts to update the base image, and `--build-arg` provides build arguments.
...
- `docker volume create {options} [{volume}]` - Creates a volume. Important options: `--driver`, `--label`, and `--opt`.

- `docker volume ls {options}` - Lists volumes. Important options: `-q`, `--filter`, and `--format`.

- `docker volume inspect {options} {volumes}` - Shows volume details. The `--format` option selects fields.

- `docker volume rm {options} {volumes}` - Removes unused volumes. The `-f` option forces the request, but volumes in use by containers cannot be removed.

- `docker volume prune {options}` - By default, removes unused local anonymous volumes. The `-a` option includes unused named volumes, `--filter` limits the scope, and `-f` skips confirmation.
...
- `docker network create {options} {network}` - Creates a network. Important options: `-d` selects the driver, `--subnet` defines the subnet, and `--internal` restricts external access.

- `docker network ls {options}` - Lists networks. Important options: `-q`, `--filter`, and `--format`.
...
Current installations use the plugin accessed through `docker compose`. The separate hyphenated `docker-compose` command corresponds to the *standalone* installation maintained mainly for compatibility and is not the recommended form for new configurations.
...
The preferred name is `compose.yaml`. `compose.yml`, `docker-compose.yaml`, and `docker-compose.yml` are also recognized, the latter two for compatibility. The former top-level `version` property is not required in current versions, which follow the **Compose Specification**.
...
- `docker compose up {options} [services]` - Builds when necessary, creates or recreates, and starts the services. Without `-d`, it aggregates the logs and keeps the terminal attached. Important options: `-d`, `--build`, `--pull`, `--force-recreate`, `--no-recreate`, `--remove-orphans`, `--scale`, and `--wait`.

- `docker compose create {options} [services]` - Creates the containers without starting them.

- `docker compose build {options} [services]` - Builds the images for services that use `build`. Important options: `--no-cache`, `--pull`, and `--build-arg`.

- `docker compose pull {options} [services]` - Downloads the declared images. Important options: `--ignore-buildable`, `--include-deps`, and `--policy`.
...
- `docker compose config {options} [services]` - Validates, combines, and shows the resolved model. Important options: `--environment`, `--images`, `--services`, `--volumes`, and `-q`.

- `docker compose ps {options} [services]` - Lists the project's containers. Important options: `-a`, `-q`, `--filter`, `--format`, and `--status`.

- `docker compose logs {options} [services]` - Shows service logs. Important options: `-f`, `--tail`, `--since`, `--until`, and `-t`.

- `docker compose exec {options} {service} {command} {arguments}` - Runs a process in an active service container. Important options: `-u`, `-e`, `-w`, and `-T`, which disables the pseudo-terminal allocated by default.

- `docker compose run {options} {service} {command} {arguments}` - Creates a separate container for a one-time task based on the service. Important options: `--rm`, `--no-deps`, `-e`, `-v`, and `--entrypoint`.
...
- `docker compose down {options} [services]` - Stops and removes containers and networks created for the project. Important options: `-v` removes declared named volumes and associated anonymous volumes, `--rmi` removes images, and `--remove-orphans` removes containers for services that are no longer in the file.
...
- `docker system df {options}` - Shows the space used by images, containers, local volumes, and the *build cache*. The `-v` option shows details.

- `docker system prune {options}` - Removes unused resources, such as stopped containers, unused networks, dangling images, and the *build cache*. Important options: `-a` extends removal to images not used by containers, `--volumes` includes anonymous volumes, `--filter` restricts the scope, and `-f` skips confirmation.
...
- `docker system events {options}` - Follows events produced by the daemon, such as container creation, start, stop, and removal. Important options: `--since`, `--until`, `--filter`, and `--format`.
